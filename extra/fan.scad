...
fan(30, 10, 24, blade_num=5);

module fan(width, depth, mount, mount_diameter=3.4, corner_radius=3, blade_angle=-45, blade_num=5)
{
  bore_diameter = width - 3;
  
  color(fan) {
    body(width, depth, bore_diameter, mount, mount_diameter, corner_radius);
    fan_blades(bore_diameter, depth, blade_angle, blade_num);
  }
}
...
module fan_blades(width, depth, angle, blade_num=5)
{ 
  $fn=50;
